...
export class Guard {
  ...
  public static againstNullOrUndefinedBulk(
    ...
    for (const arg of args) {
      const result = this.againstNullOrUndefined(arg.argument, arg.argumentPath)
      if (!result.succeeded) return result
    }
    return getSuccessResult(args[0].argumentPath)
  }
  ...
  public static containUperCase(value: string, argumentPath: string): IGuardResult {
    ...
    return {
      succeeded: false,
      message: `${argumentPath} not contain uper case`
    }
  }

  public static containLowerCase(value: string, argumentPath: string): IGuardResult {
    for (let index = 0; index < value.length; index++) {
      const element = value[index];
    ...
    return {
      succeeded: false,
      message: `${argumentPath} not contain lower case`
    }
  }
